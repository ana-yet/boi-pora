...
/** Bounds of the word under `caret` in `blockText`. Returns null if not on a word. */
export function wordBoundsAt(blockText: string, caret: number): { start: number; end: number } | null {
...
/** Sentence in `blockText` that contains the half-open range `[wordStart, wordEnd)`. */
export function sentenceContaining(blockText: string, wordStart: number, wordEnd: number): string {
  const s = blockText;
...
export function nearestBlockElement(node: Node, root: HTMLElement): HTMLElement | null {
  let n: Node | null = node.nodeType === Node.TEXT_NODE ? node.parentElement : (node as Node);
  while (n && n !== root) {
...
export function textOffsetInBlock(block: HTMLElement, textNode: Text, offsetInNode: number): number {
...
export function caretRangeFromClientPoint(doc: Document, x: number, y: number): Range | null {
